...
namespace AutoHook.IPC
{
    internal class AutoHookIPC
    {
        ...
        internal static void Init()
        {
            ...
            Service.PluginInterface.GetIpcProvider<string, object>(CreateAndSelectPresetStr)
                .RegisterAction(CreateAndSelectAnonymousPreset);
            Service.PluginInterface.GetIpcProvider<object>(DeletePresetStr).RegisterAction(DeleteSelectedPreset);
            Service.PluginInterface.GetIpcProvider<object>(DeleteAllAnonymousPresetsStr)
                .RegisterAction(DeleteAllAnonymousPresets);
        }

        private static void SetPluginState(bool state)
        {
            Service.Configuration.PluginEnabled = state;
            Service.Save();
        }

        private static void SetAutoGigState(bool state)
        {
